...
- 5cd9ee0: Light links step down to the darker brand ramp step. Body-text links also sit on the subtle surface cards paint, and there the old step read 4.48–4.49:1 — under WCAG AA, found by a consumer measuring a board card the seed surfaces all passed. The subtle surface joins the link-contrast spec's surface list (emitted per skin by the token generator, so it cannot drift), and `tone-brand` is untouched: fills keep their step, only link text moves.
...
- 5532d38: `DiagramEditor` adds nodes from a `palette` — dragged onto the canvas, or tapped / Enter for the middle of the view — as `onAddNode({ kind, position })` requests in whole cells, and renames a node in place through `editingId`, `onRenameSubmit` and `onRenameCancel`. In both `Diagram` and `DiagramEditor`, scroll and a two-finger swipe now pan; pinch, Ctrl/⌘ + scroll and the controls zoom.
- 7d39f33: `Select` sizes itself to its options: the field is as wide as its longest option (12rem–28rem, never wider than its container), the dropdown may be wider than the field up to 32rem, and a value or option that still does not fit ellipsises with its whole text in a tooltip. A consumer width, `popupMatchSelectWidth`, `labelRender` or `optionRender` still wins.

### Patch Changes

- c5cf17b: `DiagramEditor`'s inspector sits beside the canvas when there is room, as `renderInspector` has claimed since it shipped (ceebee-ui#24). The editor's body was both the size container and the element its `@container` rule targeted, and an element cannot query its own size — so the rule never matched and the inspector stayed under the canvas at every width. The editor itself is the container now.
- c5cf17b: `Select`'s stylesheet actually ships. `build-css.mjs` concatenates the CSS of named source folders, and Select sat outside every one of them, so its width cap and ellipsis never reached `dist/styles.css` — a form at 360px scrolled sideways for want of them. Select moves into a bundled folder, and the folder list moves to `css-groups.mjs`, shared by the build and a spec that fails when any stylesheet sits outside it, so the next one cannot go missing quietly.
...
- ece09a3: Board cards can be opened without a button on the front. Pass `onCardOpen` and the card's **title** becomes the open affordance — click it, or focus it and press Enter or Space — so a card front can be a title and badges rather than a row of action buttons.

  The open lives on the title, not the whole card, for the same reason `handle` exists: a card that is itself a control has presentational children, so a `meta` button inside it would leave the accessibility tree. Enter and Space are taken by the open button and stopped from reaching the card's move path, so one press never opens _and_ picks a card up. A card with no `onCardOpen` is unchanged.

- 475abf9: `Schedule`: a new Composition that draws items against a time axis — one row per item, a bar from its planned start to its planned end, the reported progress as a fill, and a late item toned danger.
...
- 419a020: The themes declare `color-scheme`, so the browser's own chrome follows them: a dark theme no longer gets light scrollbars (a white track beside dark surfaces), light native pickers or light autofill. Scrollbars take one colour — the theme's foreground at low strength, no track — through the inherited `scrollbar-color`, and return to the system's under `forced-colors`.

## 1.12.0

### Minor Changes

- 75abef8: `TargetBars` — actual against target per category, a bullet chart. Every row shares one scale that always contains zero, so a small margin never looks large and a loss draws left of the line; the target is a mark on the same track. Both figures are written as text and the bars are `aria-hidden`; a `null` figure reads as unknown rather than zero. Server-safe, plain DOM, with forced-colors and reduced-motion renderings. `better="lower"` serves costs and delays.

## 1.11.0

### Minor Changes

- 9947ec8: `ThemeProvider` takes a `locale` — one of the locale exports such as `idIDLocale` — so the strings the component runtime draws itself ("No data", "Select date", pagination) follow the product's language, static dialogs included. The locale objects were already exported; there was no way to apply them.
...
- 38b2395: Dated charts: `TimeSeriesChart`, `ProgressCurve` and `BalanceCurve`.
  ...
  description of the first — a table of every reading, always in the DOM, and shown _instead of_ the
...
- 2f314da: Board: a Kanban Composition whose cards move by pointer and by keyboard
...
- 79ac716: Give the control ladder a coarse-pointer floor at the 44px touch target.
...
- 0376bf6: `Diagram` and `DiagramEditor`: nodes joined by arrows on a grid, built on the React Flow runtime
...
- 9c31790: `PanZoomCanvas` now offers fullscreen from its own toolbar, beside zoom and reset.
...
- f4b74ff: Map `colorTextDisabled` so a disabled control's label stays readable.
...
- 1d7ad0b: Checkbox now paints from Tokens before Ant's stylesheet lands, instead of not painting at all.
...
- 467e9b7: Restore generated Flutter parity for the link foreground and canvas block-size
  tokens already exposed by the CSS token source.

## 1.4.2

### Patch Changes

- cf321e7: The Ant pre-paint layer now matches Ant's own size and spacing, so controls no longer resize at hydration.
...
- 5419afd: The Ant pre-paint layer no longer states constraint properties, which an Ant rule cannot override.
